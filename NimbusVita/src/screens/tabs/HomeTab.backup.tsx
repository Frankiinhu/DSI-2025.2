...
const HomeTab: React.FC = () => {
  ...
  const calculateRisk = (data: WeatherData) => {
    let riskScore = 0;
    ...
    if (data.uvFromApi && data.uvIndex > 7) {
      riskScore += 20;
    ...
    if (data.pressure < 1000) {
      riskScore += 20;
      riskFactors.push('pressão muito baixa');
    ...
    if (data.windSpeed > 30) {
      riskScore += 15;
      riskFactors.push('vento muito forte');
    ...
    return { riskLevel, riskPercentage, riskFactors };
  };  useEffect(() => {
    // Tenta usar cache e buscar dados reais da API em background; se falhar, usa gerador aleatório
    const load = async () => {
      ...
      try {
        ...
        console.log('getCurrentWeather: sucesso', real);
        ...
        const risk = calculateRisk(newWeatherData);
        setStatusData({
          location: 'Recife-PE, Brasil',
          riskLevel: risk.riskLevel,
          riskPercentage: risk.riskPercentage,
          description: generateStatusDescription(newWeatherData),
          lastUpdate: new Date().toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' })
        });
  ...
  const generateStatusDescription = (data: WeatherData) => {
    ...
    // Temperatura
    if (data.temperature > 28) {
      factors.push('calor intenso');
    } else if (data.temperature < 18) {
      factors.push('frio intenso');
    }

    // Umidade
    if (data.humidity > 70) {
      factors.push('umidade alta');
    ...
    // Pressão
    if (data.pressure < 1000) {
      factors.push('pressão muito baixa');
    } else if (data.pressure < 1010) {
      factors.push('pressão baixa');
    }

    // UV
    if (data.uvFromApi && data.uvIndex > 8) {
      factors.push('UV muito alto');
    } else if (data.uvFromApi && data.uvIndex > 6) {
      factors.push('UV alto');
    }

    // Qualidade do ar
    if (data.airQuality > 150) {
      factors.push('ar muito poluído');
    } else if (data.airQuality > 100) {
      factors.push('ar poluído');
    }

    // Vento
    if (data.windSpeed > 30) {
      factors.push('vento muito forte');
    } else if (data.windSpeed > 20) {
  ...
  const calculateRisk = (data: WeatherData) => {
    ...
    const riskPercentage = Math.min(riskScore, 90);
    let riskLevel = riskPercentage < 30 ? 'Baixo' : 
                    riskPercentage < 60 ? 'Moderado' : 'Alto';
  ...
  const updateStatus = (data: WeatherData) => {
    ...
    // Gera a descrição do status e atualiza
    setStatusData({
      location: 'Recife-PE, Brasil',
      riskLevel,
      riskPercentage,
          ...
          <View style={{ marginBottom: 16 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <View style={{ marginLeft: 12, flexDirection: 'row', alignItems: 'center' }}>
                <View style={{
                  backgroundColor: weatherSource === 'api' ? '#dff6e0' : '#f0f0f0',
                  paddingHorizontal: 8,
                  paddingVertical: 4,
                  borderRadius: 12,
                }}>
                  <Text style={{ color: weatherSource === 'api' ? '#216e2b' : '#666', fontSize: 12 }}>{weatherSource === 'api' ? 'API' : 'Simulado'}</Text>
                </View>
                <TouchableOpacity onPress={reloadWeather} style={{ marginLeft: 8, paddingHorizontal: 8, paddingVertical: 6 }}>
                  <Text style={{ color: Colors.primary, fontSize: 13 }}>Recarregar</Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
